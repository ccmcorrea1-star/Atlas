...
Atualizado após o bloco `restart_interrupted`/`session.interrupted` do recovery Telegram.
...
- [ ] Lifecycle público do Runtime: `session.interrupted` já é consumido pelo Telegram como aviso persistente; `runtime.restarting`, `runtime.ready`, `operation.resuming` e `operation.resumed` ainda não são consumidos como handoff completo.
...
3. **Recovery Telegram:** o prompt persistido com `Retomar (confirmar risco)`/`Descartar` e o aviso idempotente de `session.interrupted` já existem, sem replay silencioso. Falta consumir `runtime.restarting`, `runtime.ready`, `operation.resuming` e `operation.resumed`; prefixar redelivery potencialmente duplicada.
