...
pub(crate) use crate::width::display_width;
...
/// Compatibility wrapper for Atlas history cells and composer layout.
...
/// Compatibility wrapper for multiline textarea and footer layout.
pub(crate) fn wrap_text(input: &str, width: usize) -> Vec<String> {
    input
        .split('\n')
        .flat_map(|line| {
            let source = ratatui::text::Line::from(line.to_owned());
            word_wrap_line(&source, RtOptions::new(width.max(1)))
                .into_iter()
...
/// Compatibility wrapper for command rendering without hyphenation.
...
/// Compatibility cursor calculation for the existing textarea contract.
...
/// Compatibility mapping from a display column to a UTF-8 source offset.
