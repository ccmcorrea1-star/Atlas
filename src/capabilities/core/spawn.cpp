...
void setDuration(SpawnResult& result, std::chrono::steady_clock::time_point started) {
  result.duration = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::steady_clock::now() - started);
}

SpawnResult requestError(
    std::string message,
...
void prepareChild(const SpawnRequest& request, const Pipes& pipes, char* const argv[]) {
...
SpawnResult spawn(
  ...
  if (request.program.empty()) {
    return requestError("program cannot be empty", started);
  }
  if (containsNull(request.program) ||
      (request.cwd.has_value() && containsNull(request.cwd.value()))) {
    return requestError("program and cwd cannot contain NUL bytes", started);
  }
  for (const std::string& argument : request.args) {
    if (containsNull(argument)) {
      return requestError("arguments cannot contain NUL bytes", started);
    }
  }
  if (request.timeout.has_value() && request.timeout->count() < 0) {
    return requestError("timeout must be omitted or non-negative", started);
  }
  ...
  if (!createPipe(stdout_pipe)) {
    return requestError("failed to create stdout pipe: " + errnoMessage(errno), started);
  }
  if (!createPipe(stderr_pipe)) {
    close(stdout_pipe[0]);
    close(stdout_pipe[1]);
    return requestError("failed to create stderr pipe: " + errnoMessage(errno), started);
  }
  if (!createPipe(error_pipe)) {
    ...
    close(stderr_pipe[0]);
    close(stderr_pipe[1]);
    return requestError("failed to create process error pipe: " + errnoMessage(errno), started);
  }
